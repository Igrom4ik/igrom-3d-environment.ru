...
function validateAvatarField(settings: any) {
  const avatar = settings?.person?.avatar;
  if (avatar === undefined || avatar === null) return null;
  if (typeof avatar !== "string") return "Invalid settings payload";
  if (!isSafePublicPath(avatar)) return "Invalid settings payload";
  if (avatar !== "/images/avatar.jpg" && !isUploadsAvatarPath(avatar)) return "Invalid settings payload";
  return null;
}
...
async function applyAvatarModeAndSave({
  ...
  const avatarValidationError = validateAvatarField(nextSettings);
  if (avatarValidationError) {
    return { error: avatarValidationError, status: 400 as const };
  }
...
export async function PUT(request: Request) {
  try {
    ...
    try {
      ...
      const avatarValidationError = validateAvatarField(body);
      if (avatarValidationError) {
